# Agentic Awesome Skills 入门指南 (V11.5.0)
...
默认情况下，这会克隆到 `~/.agents/skills`。使用 `--cursor`、`--claude`、`--gemini`、`--codex` 或 `--kiro` 为特定工具安装，或使用 `--path <dir>` 指定自定义位置。运行 `npx agentic-awesome-skills --help` 查看详细信息。
